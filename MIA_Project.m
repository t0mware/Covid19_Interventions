...
%Make viewing time frame the duration of each method
for Strat = 1:NumStrats
t_span{Strat} = [1:Duration(Strat,1)];
end
...
hold on
for Strat = 1:NumStrats
plot(t_span{Strat},I1Mat{Strat}(1,t_span{Strat})+I2Mat{Strat}(1,t_span{Strat}))
end
...
hold on
for Strat = 1:NumStrats
plot(t_span{Strat},RMat{Strat}(1,t_span{Strat})+VMat{Strat}(1,t_span{Strat}))
end
...
hold on
for Strat = 1:NumStrats
plot(t_span{Strat},DMat{Strat}(1,t_span{Strat}))
end
legend('No intervention','Vaccination','6 month lockdown','Lockdown until vaccine','Full duration','Month based','R number','location','southeast')
xlabel('Time (Days)')
ylabel('Deaths')
...
for r = 1:iterations
    for Strat = 1:NumStrats
        for i=1:length(t_Yr)-1
            ...
            high_person_years_annual(i) = trapz(Classes.t([t_Yr(i):min(t_Yr(i+1),Duration(Strat,r))]), I1Mat{Strat}(r,[t_Yr(i):min(t_Yr(i+1),Duration(Strat,r))]))/365;
            low_person_years_annual(i) = trapz(Classes.t([t_Yr(i):min(t_Yr(i+1),Duration(Strat,r))]), I2Mat{Strat}(r,[t_Yr(i):min(t_Yr(i+1),Duration(Strat,r))]))/365;
            YLD(i) = dw*high_person_years_annual(i) + 0.2*dw*low_person_years_annual(i);
            %Costs
            range = [t_Yr(i):min(t_Yr(i+1),Duration(Strat,r))];
            %Vaccine is cost of vaccination per person plus a £600m cost in
            %first year for finding a vaccine
            if isempty(range) ==0
                vaccines(i) = CVMat{Strat}(r,range(end))-CVMat{Strat}(r,range(1))
            else
                vaccines(i) = 0
            end
            Cost_vac(i) = vacc(r)*vaccines(i) + isequal(i,1)*600e6;
            %Cost of lockdown is the cost of each day in lockdown
            %multiplied by the lockdown rate
            Cost_lock(i) = lock_day(r)*sum(LRMat{Strat}(r,range));
...
%Draw on ICERs
plot(MeanDDALY([1 2 4 5])./1e3,MeanDCost([1 2 4 5])./1e9,'-k')
...
ICER(4)=(MeanDCost_disc(4)-MeanDCost_disc(3))/(MeanDDALY_disc(4)-MeanDDALY_disc(3));
ICER(5)=(MeanDCost_disc(5)-MeanDCost_disc(4))/(MeanDDALY_disc(5)-MeanDDALY_disc(4));
ICER(6)=(MeanDCost_disc(6)-MeanDCost_disc(5))/(MeanDDALY_disc(6)-MeanDDALY_disc(5));
ICER(7)=(MeanDCost_disc(7)-MeanDCost_disc(6))/(MeanDDALY_disc(7)-MeanDDALY_disc(6));
...
%Draw on ICERs
plot(MeanDDALY_disc([1 2 4 5])./1e3,MeanDCost_disc([1 2 4 5])./1e9,'-k')
...
% Strategy 1
x=0:round(ICER(2)/1e3);
scatter(x,ProbCE(1,x+1),[],CMap(1,:),'filled')
% Strategy 2
x=round(ICER(2)/1e3):round(ICER(4)/1e3);
scatter(x,ProbCE(2,x+1),[],CMap(2,:),'filled')
% Strategy 5
x=round(ICER(4)/1e3):round(ICER(5)/1e3);
scatter(x,ProbCE(4,x+1),[],CMap(4,:),'filled')
% Strategy 6
x=round(ICER(5)/1e3):max(w);
scatter(x,ProbCE(5,x+1),[],CMap(5,:),'filled')
...
xlim([0 200])
ylim([0 1])
legend([p;pp],'No intervention','Vaccination','6 month lockdown','Lockdown until vaccine','Full duration','Month based','R number','CEAF','location','NorthEast')
